...
const STATUS_COLORS: Record<string, string> = {
  ...
  "Payment Pending": "hsl(var(--destructive))",
};
...
const Dashboard = () => {
  ...
  const activeWorks = data.works.filter(
    (w) => w.status === "In Progress" || w.status === "Payment Pending",
  ).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">
              সামগ্রিক আর্থিক অবস্থান / Overview
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              ক্লায়েন্ট, কাজ ও লেনদেনের সারসংক্ষেপ এক নজরে
            </p>
          </div>
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          label="Total Clients / ক্লায়েন্ট"
          value={String(data.clients.length)}
          icon={<Users className="h-5 w-5" />}
          tone="primary"
        />
        <StatCard
          label="Active Works / চলমান কাজ"
          value={String(activeWorks)}
          icon={<Briefcase className="h-5 w-5" />}
          tone="info"
        />
        <StatCard
          label="Pending Payments / বকেয়া পেমেন্ট"
          value={String(totals.pendingPayments)}
          icon={<Clock className="h-5 w-5" />}
          tone="warning"
        />
        <StatCard
          label="Net Position / নিট অবস্থান"
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="card-surface p-5">
          <h3 className="mb-4 text-sm font-semibold">
            Category-wise Finance / ক্যাটাগরি হিসাব
          </h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="card-surface p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">
              Upcoming Due Dates / আসন্ন ডেডলাইন
            </h3>
          </div>
          <div className="space-y-2">
            {reminders.length === 0 && (
              <p className="text-sm text-muted-foreground">
                কোনো আসন্ন ডেডলাইন নেই।
              </p>
            )}
            {reminders.map((w) => {
        ...
        <div className="card-surface p-5">
          <h3 className="mb-4 text-sm font-semibold">
            Top Outstanding / সর্বোচ্চ পাওনা
          </h3>
          <div className="space-y-2">
            {data.clients
              .map((c) => ({
            ...
            {!data.clients.some(
              ...
              <p className="text-sm text-muted-foreground">কোনো পাওনা নেই।</p>
            )}
          </div>
        </div>
      </section>
